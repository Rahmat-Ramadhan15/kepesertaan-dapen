...
<body class="out-quart">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="card shadow">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                <h5 class="mb-0">Form Cetak Laporan Iuran dan Peserta</h5>
                                <a href="{{ route('operator.hitung.index') }}" class="btn btn-secondary">
                            ...
                            <div class="card-body">
                                <div class="container">
                                    <h4 class="mb-4">Preview Laporan @switch($jenis)
                                        @case('daftar_iuran') Daftar Iuran Peserta @break
                                        @case('daftar_peserta') Daftar Peserta per Iuran @break
                                        @case('pk_dan_peserta') Rekap Iuran Pemberi Kerja & Peserta @break
                                        @default Jenis Laporan Tidak Dikenal
                                    @endswitch
                                    </h4>
                                    ...
                                    @if($data->isEmpty())
                                        <div class="alert alert-warning">Tidak ada data untuk bulan dan tahun yang dipilih.</div>
                                    @else
                                                ...
                                                <tbody>
                                                    @foreach($data as $item)
                                                        <tr>
                                                            ...
                                                            <td>{{ $item->bulan }}</td>
                                                            <td>{{ $item->tahun }}</td>
                                                            <td>Rp {{ number_format($item->iuran_peserta, 0, ',', '.') }}</td>
                                                            <td>Rp {{ number_format($item->iuran_pemberi_kerja, 0, ',', '.') }}</td>
                                                        </tr>
                                                    @endforeach
                                                </tbody>
                                            </table>

                                        @elseif($jenis == 'daftar_peserta')
                                            <table class="table table-bordered">
                                                <thead>
                                                    <tr>
                                                        <th>Nama</th>
                                                ...
                                                <tbody>
                                                    @foreach($data->groupBy('nip') as $nip => $items)
                                                        <tr>
                                                            <td>{{ $items->first()->peserta->nama ?? '-' }}</td>
                                                            <td>{{ $nip }}</td>
                                                            <td>{{ $items->first()->peserta->kode_cabang ?? '-' }}</td>
                                                            <td>
                                                                Rp {{
                                                                    number_format(
                                                                        $items->sum('iuran_peserta') + $items->sum('iuran_pemberi_kerja'),
                                                                        0, ',', '.'
                                                                    )
                                                                }}
                                                            </td>
                                                ...
                                                <tbody>
                                                    @foreach($data as $item)
                                                        <tr>
                                                            ...
                                                            <td>Rp {{ number_format($item->iuran_peserta, 0, ',', '.') }}</td>
                                                            <td>Rp {{ number_format($item->iuran_pemberi_kerja, 0, ',', '.') }}</td>
                                                            <td>Rp {{ number_format($item->iuran_peserta + $item->iuran_pemberi_kerja, 0, ',', '.') }}</td>
                                                        </tr>
                                                    @endforeach
                                                </tbody>
                                            </table>
                                        ...
                                        @else
                                            <div class="alert alert-danger">Jenis laporan tidak dikenali.</div>
                                        @endif
                                    @endif
                                </div>
                                ...
                                function downloadPDF() {
                                    ...
                                    form.method = 'POST';
                                    form.action = '{{ route("generate-pdf") }}'; // Route PDF-mu
                                    ...
                                    const bulan = document.createElement('input');
                                    bulan.type = 'hidden';
                                    bulan.name = 'bulan';
                                    bulan.value = '{{ request("bulan") }}';
                                    form.appendChild(bulan);

                                    const tahun = document.createElement('input');
                                    tahun.type = 'hidden';
                                    tahun.name = 'tahun';
                                    tahun.value = '{{ request("tahun") }}';
                                    form.appendChild(tahun);

                                    @if(request('kode_cabang'))
                                        const cabang = document.createElement('input');
                                        cabang.type = 'hidden';
                                        cabang.name = 'kode_cabang';
                                        cabang.value = '{{ request("kode_cabang") }}';
                                        form.appendChild(cabang);
                                    @endif

                                    // Tambahkan semua NIP yang tampil di preview
                                    @foreach ($data as $item)
                                        const nipInput{{ $loop->index }} = document.createElement('input');
                                        nipInput{{ $loop->index }}.type = 'hidden';
                                        nipInput{{ $loop->index }}.name = 'nips[]';
                                        nipInput{{ $loop->index }}.value = '{{ $item->peserta->nip }}';
                                        form.appendChild(nipInput{{ $loop->index }});
                                    @endforeach
                ...
                <div class="header__brand">
                    <div class="brand-wrap">
                        ...
                        <a href="{{ route('operator.dashboard') }}" class="brand-img stretched-link">
                            <img src="{{ asset('images/sulselbar.png') }}" alt="Nifty Logo" class="Nifty logo"
                                width="36" height="36">
                        </a>
                ...
                <div class="mainnav__top-content scrollable-content pb-5">
                    ...
                    <div class="mainnav__categoriy py-3">
                        <ul class="mainnav__menu nav flex-column">
                            ...
                            <li class="nav-item">
                                <a href="{{ route('operator.parameters.databank') }}"
                                    class="nav-link {{ request()->routeIs('operator.parameters.databank') ? 'active' : '' }}">
                                    <i class="fas fa-landmark me-2"></i> <span class="nav-label">Data Bank</span>
                                </a>
                            </li>
                            <li class="nav-item">
                                <a href="{{ route('operator.parameters.datacabang') }}"
                                    class="nav-link {{ request()->routeIs('operator.parameters.datacabang') ? 'active' : '' }}">
                            ...
                            <li class="nav-item has-sub">
                                ...
                                <ul class="mininav-content nav collapse">
                                    ...
                                    <li class="nav-item">
                                        <a href="{{ route('operator.parameters.nilaisekarang') }}"
                                            class="nav-link {{ request()->routeIs('operator.parameters.nilaisekarang') ? 'active' : '' }}">
                                            <i class="fas fa-chart-line me-2"></i> Nilai Sekarang
                                        </a>
                                    </li>
                                    <li class="nav-item">
                                        <a href="{{ route('operator.parameters.nsanak') }}"
                                            class="nav-link {{ request()->routeIs('operator.parameters.nsanak') ? 'active' : '' }}">
                                            <i class="fas fa-child me-2"></i> NS Anak
                                        </a>
                                    </li>
                                    <li class="nav-item">
                                        <a href="{{ route('operator.parameters.nsjanda') }}"
                                            class="nav-link {{ request()->routeIs('operator.parameters.nsjanda') ? 'active' : '' }}">
                                            <i class="fas fa-female me-2"></i> NS Janda
                                        </a>
                                    </li>
                                    <li class="nav-item">
                                        <a href="{{ route('operator.parameters.nspegawai') }}"
                                            class="nav-link {{ request()->routeIs('operator.parameters.nspegawai') ? 'active' : '' }}">
                            ...
                            <li class="nav-item">
                                <a href="{{ route('operator.parameters.ptkp') }}"
                                    class="nav-link {{ request()->routeIs('operator.parameters.ptkp') ? 'active' : '' }}">
                            ...
                            <li class="nav-item">
                                <a href="{{ route('cetak.index') }}"
                                    class="nav-link {{ request()->routeIs('cetak.index') ? 'active' : '' }}">
                            ...
                            <li class="nav-item">
                                <a href="{{ route('hitung.index') }}"
                                    class="nav-link active {{ request()->routeIs('hitung.index') ? 'active' : '' }}">
    ...
    <!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ -->
    <div id="_dm-settingsContainer" class="_dm-settings-container offcanvas offcanvas-end rounded-start"
        tabindex="-1">
                    ...
                    <div class="d-flex align-items-center pt-3 mb-2">
                        ...
                        <div class="form-check form-switch">
                            <input id="_dm-miniNavRadio" class="form-check-input ms-0" type="radio"
                                name="navigation-mode" autocomplete="off">
                        </div>
                    </div>
                    ...
                    <div class="d-flex align-items-center pt-1 mb-2">
                        ...
                        <div class="form-check form-switch">
                            <input id="_dm-maxiNavRadio" class="form-check-input ms-0" type="radio"
                                name="navigation-mode" autocomplete="off" checked>
                        </div>
                    </div>
                ...
                <div class="col-lg-6 p-4">
                    ...
                    <div class="d-flex mb-4 pb-4">
                        <div class="d-flex flex-column">
                            ...
                            <div class="form-check form-check-alt form-switch">
                                <input id="settingsThemeToggler" class="form-check-input mode-switcher"
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="_dm-colorShcemesMode">
                                    ...
                                    <button type="button" class="_dm-colorModeBtn btn p-1 shadow-none"
                                        ...
                                        <img src="{{ asset('assets/img/color-schemes/expanded-header.png') }}"
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="_dm-colorShcemesMode">
                                    ...
                                    <button type="button" class="_dm-colorModeBtn btn p-1 shadow-none"
                                        ...
                                        <img src="{{ asset('assets/img/color-schemes/fair-header.png') }}"
                                            alt="color scheme illusttration" loading="lazy">
                                    </button>
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="_dm-colorShcemesMode">
                                    ...
                                    <button type="button" class="_dm-colorModeBtn btn p-1 shadow-none"
                                        ...
                                        <img src="{{ asset('assets/img/color-schemes/full-header.png') }}"
                                            alt="color scheme illusttration" loading="lazy">
                                    </button>
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="_dm-colorShcemesMode">
                                    ...
                                    <button type="button" class="_dm-colorModeBtn btn p-1 shadow-none"
                                        ...
                                        <img src="{{ asset('assets/img/color-schemes/navigation.png') }}"
                                            alt="color scheme illusttration" loading="lazy">
                                    </button>
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="_dm-colorShcemesMode">
                                    ...
                                    <button type="button" class="_dm-colorModeBtn btn p-1 shadow-none"
                                        ...
                                        <img src="{{ asset('assets/img/color-schemes/brand.png') }}"
                                            alt="color scheme illusttration" loading="lazy">
                                    </button>
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="_dm-colorShcemesMode">
                                    ...
                                    <button type="button" class="_dm-colorModeBtn btn p-1 shadow-none"
                                        ...
                                        <img src="{{ asset('assets/img/color-schemes/tall-header.png') }}"
                                            alt="color scheme illusttration" loading="lazy">
                                    </button>
                    ...
                    <div class="pt-3">
                        ...
                        <div class="d-flex gap-3 my-3">
                            <label for="_dm-fontSizeRange" class="form-label flex-shrink-0 mb-0">Root Font
                                sizes</label>
                            <div class="position-relative flex-fill">
                                <input type="range" class="form-range" min="9" max="19"
                                    step="1" value="16" id="_dm-fontSizeRange">
                            ...
                            <div class="col-5">
                                ...
                                <div class="d-flex align-items-center pt-1 mb-2">
                                    <label class="form-check-label flex-fill"
                                        for="_dm-sidebarsScrollbarCheckbox">Navigation and Sidebar</label>
                                    <div class="form-check form-switch">
